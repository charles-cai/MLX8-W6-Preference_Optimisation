...
from peft import LoraConfig, get_peft_model # 💡 IMPORT: Added get_peft_model
from transformers import (
    AutoModelForCausalLM,
    AutoTokenizer,
    BitsAndBytesConfig,
    TrainingArguments,
    Trainer,  # 💡 IMPORT: Switched to the core Trainer
)
from google.colab import userdata, drive
from huggingface_hub import login

from typing import Dict, List  # 💡 FIX: Import List and Dict for type hinting
...
# 💡 NEW: Tokenize the dataset so it's ready for the standard Trainer
def tokenize_function(examples):
    return tokenizer(examples["text"], truncation=True, max_length=1024)

tokenized_dataset = formatted_dataset.map(tokenize_function, batched=True, remove_columns=["text"])

# --- 💡 NEW: Custom Data Collator for Completion-Only Loss ---
...
# 💡 NEW: Manually wrap the model with the LoRA config
...
# 💡 CHANGE: Using the standard `Trainer` now
trainer = Trainer(
    model=model,
...
# --- Start Training ---
print("🚀 Starting SFT training with the standard `transformers.Trainer`...")
trainer.train()
print("✅ Base model training complete. We did it.")
